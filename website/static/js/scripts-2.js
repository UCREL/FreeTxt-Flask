...
function resetToDefault() {
 ...
 // Clear the file input
    ...
    $("#submit-rows-btn, #loading, #tab-buttons, #tabs").addClass('hidden');
    // Reset Summary Tool
    $("#chosen_ratio").val(40);
    $("#ratio-value").text('40%');
    $("#summary").empty();
...
function displayOverallSentiment(sentimentCounts) {
    ...
    if (overallSentiment > 0) {
         if (getCurrentLanguage() === 'cy') {
            sentimentExplanation.innerText = `Mae'r sgôr sentiment ar gyfartaledd o ${overallSentiment} yn dangos fod  ${overallSentiment}  yn fwy o sentimentau cadarnhaol na rhai negyddol yn y testun penodol.  Mae hyn yn awgrymu fod sentiment y testun ar gyfartaledd yn cadarnhaol. `;
        ...
        if (getCurrentLanguage() === 'cy') {
            sentimentExplanation.innerText = `Mae'r sgôr sentiment ar gyfartaledd o ${overallSentiment} yn dangos fod  ${overallSentiment}  yn fwy o sentimentau negyddol na rhai cadarnhaol yn y testun penodol.  Mae hyn yn awgrymu fod sentiment y testun ar gyfartaledd yn negyddol. `;
...
function displayTableContent(data, tbodyElement) {
    ...
    const slicedData = data.slice((currentPagesent - 1) * itemsPerPagesent, currentPagesent * itemsPerPagesent);
    ...
    createPaginationControlsent(tbodyElement.closest('#SentimentTable'), data);
}

function createPaginationControlsent(container, data) {
    if (!container) {
        console.error("Container not provided to createPaginationControlsent function.");
        return;
    }
    ...
    paginationDiv.className = 'w3-bar';
     ...
     // Update the total pages based on the size of the current data (filtered or full)
     ...
     if (currentPagesent > 1) {
         ...
         prevButton.addEventListener('click', function() {
             ...
             createPaginationControlsent(container, data);
         });
    ...
    if (currentPagesent < totalPagessent) {
        ...
        nextButton.addEventListener('click', function() {
            currentPagesent++;
            const filterDropdown = container.querySelector('select');
...
function drawWordTree(wordTreeData, search_word) {
  ...
  document.addEventListener("DOMContentLoaded", function() {
    ...
    if(searchWordInput) {  // check if the element exists
...
function sendWordCloudRequest() {
    ...
    const selectedcloudmeasure = cloud_measureDropdown.value;
...
document.addEventListener("DOMContentLoaded", function() {
    ...
    const cloud_measureDropdown = document.querySelector('select[name="cloud_measure"]');
    cloud_measureDropdown.addEventListener('change', generateWordClouds);
...
function regenerateWordCloud() {
   ...
   const formData = new FormData(document.getElementById('wordCloudForm'));
    ...
    .then(data => {
        const wordCloudImageElement = document.getElementById('wordCloudImage');
        wordCloudImageElement.src = data.wordcloud_image_path;  // Refresh the image
    })
    .catch(error => {
...
function displayResults(data) {
    ...
    const loadingElement = document.getElementById('loading');
    loadingElement.style.display = 'none';
        // Remove 'hidden' class to make elements visible
        $('#kwicTable').removeClass('hidden');
        $('#collocsTable').removeClass('hidden');
...
function getSelectedColumns() {
    const dropdown = document.getElementById('columns-dropdown');
    const selectedOptions = Array.from(dropdown.selectedOptions); // Convert HTMLCollection to Array
    return selectedOptions.map(option => option.value); // Return an array of selected option values
}
...
function downloadWordTree() {
    ...
    const scripts = `
        ...
        <script type="text/javascript">
            ...
            function drawChart() {
                var data = google.visualization.arrayToDataTable(${JSON.stringify(currentWordTreeData)}); // Convert data to string
    ...
    }, 3000); // 5 seconds delay to give the chart enough time to render
}
